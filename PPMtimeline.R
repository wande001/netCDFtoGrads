...
for(v in var){
  for(lag in c(0:11)){
    tel = 0
    varCount = varCount + 1
    outLead[[varCount]] = array(NA, c(12,6, length(models)*length(ref)))
    for(m in models){
      for(r in ref){
        tel = tel + 1
...
for(v in var){
  for(lag in c(0:12)){
    tel = 0
    varCount = varCount + 1
    outTime[[varCount]] = array(NA, c(12,6, length(models)*length(ref)))
    for(m in models){
      for(r in ref){
        tel = tel + 1
        NC = open.ncdf(paste(m,r,v,"PPM.nc", sep="_"))
        data = get.var.ncdf(NC, paste("Time",lag, sep="_"))
        close.ncdf(NC)
        for(time in 1:12){
          temp = data[,,time]
          for(con in 1:6){
            outTime[[varCount]][time,con,tel] = mean(temp[continent == con], na.rm=T)
          }
        }
      }
    }
...
for(v in var){
  for(lag in c(0:8)){
    tel = 0
    varCount = varCount + 1
    outSpace[[varCount]] = array(NA, c(12,6, length(models)*length(ref)))
    for(m in models){
      for(r in ref){
        tel = tel + 1
        NC = open.ncdf(paste(m,r,v,"PPM.nc", sep="_"))
        data = get.var.ncdf(NC, paste("Space",lag, sep="_"))
        close.ncdf(NC)
        for(time in 1:12){
          temp = data[,,time]
          for(con in 1:6){
            outSpace[[varCount]][time,con,tel] = mean(temp[continent == con], na.rm=T)
          }
        }
      }
    }
...
pdf("../skillTimeLine.pdf", width=12, height=8)
layout(A)
parLeft = c(1,4,2,1)
parRight = c(1,1,2,4)
cols = tim.colors(13)
...
for(v in 1:length(var)){
  ...
  plot(1,1,type="n", xlim=c(0,12), ylim=c(0,6), xaxs="i", yaxs="i", axes=FALSE, ylab="", xlab="", main=paste(titles[v], "aggregation time"))
  ...
  box()
  for(i in 13:1){
    ensPPM = rowMeans(outTime[[i+(v-1)*12]], na.rm=T, dims=2)
    for(con in 1:6){
...
for(v in 1:length(var)){
  ...
  plot(1,1,type="n", xlim=c(0,12), ylim=c(0,6), xaxs="i", yaxs="i", axes=FALSE, ylab="", xlab="", main=paste(titles[v], "lead time"))
  #axis(1, labels=c("J","F","M","A","M","J","J","A","S","O","N","D"), at=seq(0.5,11.5,1))
  if(v ==1){
    axis(2, labels=c("NA","EU","AF","SA","AS","OC"), at=seq(0.5,5.5,1), las=1, hadj=1, tick=FALSE)
  }
...
for(v in 1:length(var)){
  ...
  plot(1,1,type="n", xlim=c(0,12), ylim=c(0,6), xaxs="i", yaxs="i", axes=FALSE, ylab="", xlab="", main=paste(titles[v], "aggregation space"))
  axis(1, labels=c("J","F","M","A","M","J","J","A","S","O","N","D"), at=seq(0.5,11.5,1))
  if(v ==1){
    axis(2, labels=c("NA","EU","AF","SA","AS","OC"), at=seq(0.5,5.5,1), las=1, hadj=1, tick=FALSE)
  }
  ...
  box()
  for(i in 1:9){
    ensPPM = rowMeans(outSpace[[i+(v-1)*9]], na.rm=T, dims=2)
    for(con in 1:6){
      lines(seq(0.5,11.5,1), (ensPPM[,con]+con-1), type="b", col=cols[i], pch=19)
    }
  }
