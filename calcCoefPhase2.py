...
ncOutputFile = "../resultsNetCDF/NNLS_weights_month_"+sys.argv[3]+"_var_"+varName+".nc4"
...
../resultsNetCDF/NNLS_weights_month_1.nc4
posCount = 0

for lag in range(12):
  for event in range(start,start+2):
    month = 1
    ...
    #CCSM, varCCSM = returnForecast(dateInput, endDay, "CCSM", "prec", lag)
    #ensCCSM = np.sum(CCSM, axis=0)
    #CCSMsel = np.isnan(ensCCSM[:,0,0]) == False
