...
for(temp in 0:12){
  for(lag in 0){
    NC = open.ncdf(paste(model,"_tempScale_",temp,"_lag_",lag,".nc", sep=""))
    for(spat in 0:8){
      print(spat)
      if(temp ==0){
        for(time in 1:24){
...
mv <- -9999
PPM <- var.def.ncdf( "PPM", "-", list(dimX, dimY, dimT), mv,prec="double")
nc <- create.ncdf(paste(model,"_PPM.nc",sep=""), list(PPM))

put.var.ncdf(nc, PPM, CCevents/(count/12))
